...
def _decode_rx1_models(outer: bytes, *, block_models: bool = False):
    ...
    semantic_body = semantic_stream if block_models else _decompress_brotli(semantic_stream)
    carrier_body = carrier_stream if block_models else _decompress_brotli(carrier_stream)
    ...
    if block_models:
        from .segment_layout import unpack_segments
        semantic_body = unpack_segments(semantic_body)
...
def read_residual_archive(archive_path: Path) -> ResidualArchiveParts:
    ...
    block_models = outer.startswith(b"BLK2")
    packed_outer = outer
    if block_models:
        from .block_container import decode_model_blocks
        models, tail = decode_model_blocks(outer)
        if len(models) < RX1_MODEL_HEADER.size:
            raise ResidualArchiveError("truncated block model header")
        fields = RX1_MODEL_HEADER.unpack_from(models)
        if fields[:3] != (RX1_MAGIC, 1, RX1_CODEC_BROTLI) or fields[4] & 7:
            raise ResidualArchiveError("invalid block model representation")
        if RX1_MODEL_HEADER.size + sum(fields[-3:]) != len(models):
            raise ResidualArchiveError("block model section lengths differ")
        outer = models + tail
    rx1 = _decode_rx1_models(outer, block_models=block_models)
    if rx1 is not None:
        semantic, carrier, hpac, compensation, section, compressed = rx1
        if block_models:
            compressed = packed_outer[:len(packed_outer) - len(section)]
    else:
